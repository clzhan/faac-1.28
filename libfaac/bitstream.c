...
static int WriteFAACStr(BitStream *bitStream, char *version, int write)
{
  ...
  SRPutBit(bitStream, ID_FIL, LEN_SE_ID);
  if (count < 15)
  {
	  SRPutBit(bitStream, count, 4);
  }
  else
  {
	  SRPutBit(bitStream, 15, 4);
	  SRPutBit(bitStream, count - 14, 8);
  }

  SRPutBit(bitStream, 0, padbits);
  SRPutBit(bitStream, 0, 8);
  SRPutBit(bitStream, 0, 8); // just in case
  for (i = 0; i < len; i++)
	  SRPutBit(bitStream, str[i], 8);

  SRPutBit(bitStream, 0, 8 - padbits);
...
/* sur: faad2 complains about scalefactor error if we are writing FAAC String */
    ...
    bits += LEN_SE_ID;
	SRPutBit(bitStream, ID_END, LEN_SE_ID);
                           ...
                           int writeFlag)
{
    ...
    if (writeFlag) {
        /* Fixed ADTS header */
		SRPutBit(bitStream, 0xFFFF, 12); /* 12 bit Syncword */
		SRPutBit(bitStream, hEncoder->config.mpegVersion, 1); /* ID == 0 for MPEG4 AAC, 1 for MPEG2 AAC */
		SRPutBit(bitStream, 0, 2); /* layer == 0 */
		SRPutBit(bitStream, 1, 1); /* protection absent */
		SRPutBit(bitStream, hEncoder->config.aacObjectType - 1, 2); /* profile */
		SRPutBit(bitStream, hEncoder->sampleRateIdx, 4); /* sampling rate */
		SRPutBit(bitStream, 0, 1); /* private bit */
		SRPutBit(bitStream, hEncoder->numChannels, 3); /* ch. config (must be > 0) */
                                                     /* simply using numChannels only works for
                                                        6 channels or less, else a channel
                                                        configuration should be written */
		SRPutBit(bitStream, 0, 1); /* original/copy */
		SRPutBit(bitStream, 0, 1); /* home */

#if 0 // Removed in corrigendum 14496-3:2002
        if (hEncoder->config.mpegVersion == 0)
			SRPutBit(bitStream, 0, 2); /* emphasis */
#endif

        /* Variable ADTS header */
		SRPutBit(bitStream, 0, 1); /* copyr. id. bit */
		SRPutBit(bitStream, 0, 1); /* copyr. id. start */
		SRPutBit(bitStream, hEncoder->usedBytes, 13);
		SRPutBit(bitStream, 0x7FF, 11); /* buffer fullness (0x7FF for VBR) */
		SRPutBit(bitStream, 0, 2); /* raw data blocks (0+1=1) */
                    ...
                    int writeFlag)
{
    ...
    if (writeFlag) {
		...
		SRPutBit(bitStream, ID_CPE, LEN_SE_ID);

        /* write the element_identifier_tag */
		SRPutBit(bitStream, channelInfo->tag, LEN_TAG);

        /* common_window? */
		SRPutBit(bitStream, channelInfo->common_window, LEN_COM_WIN);
    }
    ...
    if (channelInfo->common_window) {
        ...
        if (writeFlag) {
			SRPutBit(bitStream, channelInfo->msInfo.is_present, LEN_MASK_PRES);
            if (channelInfo->msInfo.is_present == 1) {
                int g;
                int b;
                for (g=0;g<numWindows;g++) {
                    for (b=0;b<maxSfb;b++) {
						SRPutBit(bitStream, channelInfo->msInfo.ms_used[g*maxSfb + b], LEN_MASK);
                    }
                }
            }
        }
                    ...
                    int writeFlag)
{
    ...
    if (writeFlag) {
		...
		SRPutBit(bitStream, ID_SCE, LEN_SE_ID);
                    ...
                    int writeFlag)
{
    ...
    if (writeFlag) {
		...
		SRPutBit(bitStream, ID_LFE, LEN_SE_ID);
                        ...
                        int writeFlag)
{
    ...
    if (writeFlag) {
		...
		SRPutBit(bitStream, 0, LEN_ICS_RESERV);  /* reserved Bit*/

        /* Write out window sequence */
		SRPutBit(bitStream, coderInfo->block_type, LEN_WIN_SEQ);  /* block type */

        /* Write out window shape */
		SRPutBit(bitStream, coderInfo->window_shape, LEN_WIN_SH);  /* window shape */
    }
    ...
    if (coderInfo->block_type == ONLY_SHORT_WINDOW){
        if (writeFlag) {
			SRPutBit(bitStream, coderInfo->max_sfb, LEN_MAX_SFBS);
            grouping_bits = FindGroupingBits(coderInfo);
			SRPutBit(bitStream, grouping_bits, MAX_SHORT_WINDOWS - 1);  /* the grouping bits */
        }
        bits += LEN_MAX_SFBS;
        bits += MAX_SHORT_WINDOWS - 1;
    } else { /* Otherwise, write out max_sfb and predictor data */
        if (writeFlag) {
			SRPutBit(bitStream, coderInfo->max_sfb, LEN_MAX_SFBL);
        }
        bits += LEN_MAX_SFBL;
#ifdef DRM
    }
    if (writeFlag) {
		SRPutBit(bitStream,coderInfo->tnsInfo.tnsDataPresent,LEN_TNS_PRES);
    }
    bits += LEN_TNS_PRES;
#endif
        if (objectType == LTP)
        {
            bits++;
            if(writeFlag)
				SRPutBit(bitStream, coderInfo->ltpInfo.global_pred_flag, 1); /* Prediction Global used */
            ...
            bits++;
            if (writeFlag)
				SRPutBit(bitStream, coderInfo->pred_global_flag, LEN_PRED_PRES);  /* predictor_data_present */
                    ...
                    int writeFlag)
{
    ...
    if (writeFlag)
		SRPutBit(bitStream, coderInfo->global_gain, LEN_GLOB_GAIN);
    bits += LEN_GLOB_GAIN;
#endif
    ...
    if (writeFlag) {
		...
		SRPutBit(bitStream, coderInfo->iLenReordSpData, LEN_HCR_REORDSD);

        /* length_of_longest_codeword */
		SRPutBit(bitStream, coderInfo->iLenLongestCW, LEN_HCR_LONGCW);
    }
...
static int WriteLTPPredictorData(CoderInfo *coderInfo, BitStream *bitStream, int writeFlag)
{
    ...
    if (ltpInfo->global_pred_flag)
    {
        ...
        if(writeFlag)
			SRPutBit(bitStream, 1, 1); /* LTP used */
        bits++;
        ...
        case SHORT_LONG_WINDOW:
            ...
            if(writeFlag)
            {
				SRPutBit(bitStream, ltpInfo->delay[0], LEN_LTP_LAG);
				SRPutBit(bitStream, ltpInfo->weight_idx, LEN_LTP_COEF);
            }
...
//            last_band = coderInfo->nr_of_sfb;
            ...
            if(writeFlag)
                for (i = 0; i < last_band; i++)
					SRPutBit(bitStream, ltpInfo->sfb_prediction_used[i], LEN_LTP_LONG_USED);
            break;
    ...
    if (writeFlag) {
        if (predictorDataPresent) {
            ...
            if (coderInfo->reset_group_number == -1) {
				SRPutBit(bitStream, 0, LEN_PRED_RST); /* No prediction reset */
            } else {
				SRPutBit(bitStream, 1, LEN_PRED_RST);
				SRPutBit(bitStream, (unsigned long)coderInfo->reset_group_number,
                    LEN_PRED_RSTGRP);
            }

            for (b=0;b<numBands;b++) {
				SRPutBit(bitStream, coderInfo->pred_sfb_flag[b], LEN_PRED_ENAB);
            }
        }
    }
                          ...
                          int writeFlag)
{
    ...
    if (writeFlag) {
		SRPutBit(bitStream, 0, LEN_PULSE_PRES);  /* no pulse_data_present */
    }
                        ...
                        int writeFlag)
{
...
#ifndef DRM
    if (writeFlag) {
		SRPutBit(bitStream, tnsInfoPtr->tnsDataPresent, LEN_TNS_PRES);
    }
    bits += LEN_TNS_PRES;
#endif
    ...
    for (w=0;w<numWindows;w++) {
        ...
        if (writeFlag) {
			SRPutBit(bitStream, numFilters, len_tns_nfilt); /* n_filt[] = 0 */
        }
        if (numFilters) {
            bits += LEN_TNS_COEFF_RES;
            resInBits = windowDataPtr->coefResolution;
            if (writeFlag) {
				SRPutBit(bitStream, resInBits - DEF_TNS_RES_OFFSET, LEN_TNS_COEFF_RES);
            }
            ...
            for (filtNumber=0;filtNumber<numFilters;filtNumber++) {
                ...
                if (writeFlag) {
					SRPutBit(bitStream, tnsFilterPtr->length, len_tns_length);
					SRPutBit(bitStream, order, len_tns_order);
                }
                if (order) {
                    bits += (LEN_TNS_DIRECTION + LEN_TNS_COMPRESS);
                    if (writeFlag) {
						SRPutBit(bitStream, tnsFilterPtr->direction, LEN_TNS_DIRECTION);
						SRPutBit(bitStream, tnsFilterPtr->coefCompress, LEN_TNS_COMPRESS);
                    }
                    ...
                    if (writeFlag) {
                        ...
                        for (i=1;i<=order;i++) {
							...
							SRPutBit(bitStream, unsignedIndex, bitsToTransmit);
                        }
                    }
                }
            }
                                ...
                                int writeFlag)
{
    ...
    if (writeFlag) {
		SRPutBit(bitStream, 0, LEN_GAIN_PRES);
    }
    ...
    if (writeFlag) {
        for(i = 0; i < coderInfo->spectral_count; i++) {
            if (len[i] > 0) {  /* only send out non-zero codebook data */
				SRPutBit(bitStream, data[i], len[i]); /* write data */
                bits += len[i];
            }
        }
    } else {
                            ...
                            int writeFlag)
{
    ...
    while (numberOfBitsLeft >= minNumberOfBits)
    {
        ...
        if (writeFlag) {
			SRPutBit(bitStream, ID_FIL, LEN_SE_ID);   /* Write fill_element ID */
        }
        ...
        if (numberOfBytes < maxCount) {
            int i;
            if (writeFlag) {
				SRPutBit(bitStream, numberOfBytes, LEN_F_CNT);
                for (i = 0; i < numberOfBytes; i++) {
					SRPutBit(bitStream, 0, LEN_BYTE);
                }
            }
            ...
            int i;
            if (writeFlag) {
				SRPutBit(bitStream, maxCount, LEN_F_CNT);
            }
            ...
            if (writeFlag) {
				SRPutBit(bitStream, escCount, LEN_BYTE);
...
int SRPutBit(BitStream *bitStream,
           unsigned long data,
           int numBit)
{
...
{ /* data can be written at an arbitrary position in the bitstream */
	...
	return SRPutBit(bitStream, data, numBit);
}
